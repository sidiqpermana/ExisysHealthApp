...
public class WeightFragment extends BaseFragment{
    private ArrayList<WeightItem> listItem;
    ...
    public void parsingResponse(String response) {
        ...
        listItem = Weight.getListItem(response);
        if (listItem.size() > 0){
            WeightAdapter adapter = new WeightAdapter(getActivity(), listItem);
    ...
    private void getHrmData(final String fdate1, final String fdate2) {
        ...
        if(!fdate1.equals("")&&!fdate2.equals("")){
            url = URLs.GET_WEIGHT + "between/";
            isBetween = true;
        }else{
            url = URLs.GET_WEIGHT + "by/year";
            fdate = fdate1;
            isBetween = false;
        }
